...
function validateManualInput(rate: number, discount: number, selectedDays: string[], recurring: boolean) {
  if (rate < 0) return "Rate must be ≥ 0";
  if (discount < 0 || discount > 100) return "Discount must be 0–100";
  if (recurring && discount > 0 && (!selectedDays || selectedDays.length === 0)) {
    return "Select at least one day";
  }
  return null;
}

function DaySelector({ selectedDays, onToggle, title }: { selectedDays: string[]; onToggle: (day: string) => void; title: string }) {
  return (
    <div className="flex flex-wrap gap-2 pt-2">
      {days.map((day) => (
        <button
          ...
          className={`w-8 h-8 rounded-full border text-sm font-medium transition-colors ${selectedDays.includes(day) ? "bg-[#fea632] border-[#fea632] text-white" : "bg-white border-gray-300 text-gray-500 hover:bg-gray-100"
            }`}
...
function RentalTypeForm({ title, rental, setRental, currency, error }: any) {
  const newPrice = useMemo(() => ((Number(rental.rate) * (100 - Number(rental.discount))) / 100).toFixed(2), [rental.rate, rental.discount]);
  const toggleDay = (day: string) =>
    setRental((prev: any) => ({
      ...prev,
      selectedDays: prev.selectedDays.includes(day) ? prev.selectedDays.filter((d: string) => d !== day) : [...prev.selectedDays, day],
    }));
    ...
    <div className="bg-gray-50 rounded-xl p-4 mb-5 shadow-sm border">
      <div className="flex justify-between items-center mb-1">
        ...
        <div className="flex items-center gap-2">
          ...
          <Switch checked={rental.recurring} onCheckedChange={(value) => setRental({ ...rental, recurring: value })} className="data-[state=checked]:bg-[#fea632]" />
        </div>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
        <div className="flex flex-col gap-1">
          <Label className="text-xs font-medium">Mileage</Label>
          <NoSpinnerInput value={rental.mileage} onChange={(e: any) => setRental({ ...rental, mileage: e.target.value })} placeholder="0" />
        </div>
        <div className="flex flex-col gap-1">
          <Label className="text-xs font-medium">{`Rate (${currency})`}</Label>
          <NoSpinnerInput value={rental.rate} onChange={(e: any) => setRental({ ...rental, rate: e.target.value })} placeholder="0" />
        </div>
        <div className="flex flex-col gap-1">
          <Label className="text-xs font-medium">Discount (%)</Label>
          <Select value={String(rental.discount)} onValueChange={(value) => setRental({ ...rental, discount: Number(value) })}>
            <SelectTrigger className="h-10 rounded-lg w-full">
              <SelectValue placeholder="Select" />
            </SelectTrigger>
            <SelectContent>{[...Array(31).keys()].map((val) => <SelectItem key={val} value={String(val)}>{val}%</SelectItem>)}</SelectContent>
          </Select>
        </div>
        <div className="flex flex-col gap-1">
          <Label className="text-xs font-medium">New Price</Label>
          <input type="text" value={newPrice} disabled readOnly className="h-10 rounded-lg px-3 py-2 border-gray-200 bg-gray-100 text-gray-500 w-full" />
        </div>
      </div>
      <DaySelector selectedDays={rental.selectedDays} onToggle={toggleDay} title={title} />
      {error && <p className="text-red-500 text-xs mt-2">{error}</p>}
    </div>
  );
}

export default function RateManagerCard({ car, refetch }: { car: any, refetch: any }) {
  const { appCountry } = useAgentContext();
  const defaultCurrency = appCountry === "in" ? "INR" : appCountry === "ae" ? "AED" : "USD";
  ...
  const applyChanges = async () => {
    ...
    const dailyValidation = validateManualInput(Number(daily.rate), Number(daily.discount), daily.selectedDays, daily.recurring);
    const weeklyValidation = validateManualInput(Number(weekly.rate), Number(weekly.discount), weekly.selectedDays, weekly.recurring);
    const monthlyValidation = validateManualInput(Number(monthly.rate), Number(monthly.discount), monthly.selectedDays, monthly.recurring);
    ...
    <div className="rounded-2xl shadow-lg bg-white p-4 sm:p-6 w-full max-w-lg flex flex-col space-y-4">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <img src={car.imageUrl} alt={car.name} className="w-20 h-16 object-cover rounded-lg" />
          <div>
      ...
      <RentalTypeForm title="Daily" rental={daily} setRental={setDaily} currency={currency} error={dailyError} />
      <RentalTypeForm title="Weekly" rental={weekly} setRental={setWeekly} currency={currency} error={weeklyError} />
      <RentalTypeForm title="Monthly" rental={monthly} setRental={setMonthly} currency={currency} error={monthlyError} />
