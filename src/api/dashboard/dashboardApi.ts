...
export const fetchComapnyCountry = async (userId: string) => {
  try {
    const data: any = await API.get({ slug: `users/country/${userId}` });

    if (!data) {
      throw new Error("Failed to fetch Comapny Country data");
    }
