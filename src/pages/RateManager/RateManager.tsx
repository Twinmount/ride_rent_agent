...
import { RentalType, SingleVehicleType } from "@/types/API-types";
...
const useVehicles = ({ page = 1, limit = 10, search, filter, userId }: UseVehiclesParams) =>
  useQuery({
...
export default function RateManager() {
  ...
  const { data, isLoading, isRefetching, refetch } = useVehicles({ page: 1, limit: 100, filter: "ALL", userId });

  const mappedCars = useMemo(() => {
    const vehicles: SingleVehicleType[] = data?.result?.list || [];
    ...
    return vehicles.map((vehicle: SingleVehicleType) => {

      const getRental = (type: "Daily" | "Weekly" | "Monthly"): RentalType => {
        const key = type === "Daily" ? "day" : type === "Weekly" ? "week" : "month";
        ...
        if (hasRealManualOverride) {
          return {
            ...
            weekdays: (manualOverride.weekdays || []).map(day => dayConversionMap[day] || day).filter(Boolean),
        ...
        if (bulkRule) {
          const bulkDiscountValue = (bulkRule as any)[`${type.toLowerCase()}Discount`] as number || 0;
          return {
            type,
            rate: baseRate,
            mileage: baseMileage,
            discount: bulkDiscountValue,
            weekdays: (bulkRule.applicableDays || []).map(day => dayConversionMap[day] || day).filter(Boolean),
  ...
  return (
    <div className="p-4 sm:p-6 bg-[#f7f8fa] min-h-screen">
      ...
      <h2 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6">Rate Manager</h2>

      <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 mb-4 sm:mb-6 border-b">
        <button
          className={`px-3 py-2 sm:px-4 font-semibold ${activeTab === "bulk" ? "border-b-2 border-[#fea632] text-[#fea632]" : "text-gray-500"
            }`}
        ...
        <button
          className={`px-3 py-2 sm:px-4 font-semibold ${activeTab === "manual" ? "border-b-2 border-[#fea632] text-[#fea632]" : "text-gray-500"
            }`}
      ...
      {activeTab === "bulk" && (
        <section className="bg-white rounded-xl shadow-sm p-4 sm:p-6 mb-6 sm:mb-8">
          <h3 className="text-lg sm:text-xl font-bold mb-2 sm:mb-4">Set Fleet-Wide Discounts</h3>
          <BulkDiscountEditor />
        </section>
      )}
      ...
      {activeTab === "manual" && (
        <section className="bg-white rounded-xl shadow-sm p-4 sm:p-6">
          <h3 className="text-lg sm:text-xl font-bold mb-2 sm:mb-4">Adjust Rates for Individual Vehicles</h3>
          {finalIsLoading ? (
